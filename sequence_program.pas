{$I \AC_CONF\TMP\PASCBEGI.PAS}
    ...
    var     act,act1,c11,c12,c13: byte;
            ...
            v                   : array[1..6] of byte;
            edrrs,E310,E410     : byte;
            edrst,edrin,edrsp   : byte;
            edlst,edlin,edlsp   : byte;
    
    
    procedure RA1SQ22P                                (var enx : byte;      (* END FROM LOOP *)
   ...
   begin
            case i of
                ...
                3: Tx2:='Tank RA1TKB310 empty          ';
                4: Tx2:='Tank RA1TKB410 full           ';
                5: Tx2:='                              ';
                6: Tx2:='                              ';
            end;
   end;
   ...
   begin
    ...
    begin
        ...
        act:=get__RA1SQ22_act_byte_2s and 1;
        Ost:=get__RA1SQ22_Ost_word_2s;
        ...
        if (ost=10) then begin
            if odd(fse) then v[ 1]:=get__RA1EMJ811_rng_byte_2s;
            if odd(sse) then v[ 1]:=get__RA1EMJ812_rng_byte_2s;
        end;      
        (*fse:=get__RA1EME311C_fse_byte_2s;*)
        (*sse:=get__RA1EME311C_sse_byte_2s;*)
        v[ 1]:=1;
        if (ost=10) then begin
        v[ 1]:=get__RA1EMB311_rng_byte_2s;
        end;
        v[ 2]:=1;
        v[ 3]:=(not get__RA1SQ22_cte_byte_2s);
